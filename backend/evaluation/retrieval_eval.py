...
def evaluate_iching_retrieval(col, k: int = 3) -> dict:
    ...
    for h in hexagrams:
        query = f"{h['chinese']} {h['pinyin']} {h['english']} hexagram"
        results = col.query(
            query_texts=[query],
            n_results=k,
            where={"system": "iching"},
        ...
        retrieved_ids = set(results["ids"][0])
        # Check if any of the hexagram's docs appear (judgment, symbolic, or image)
        expected_ids = {
...
def main():
    ...
    for name, func in [
        ...
        ("I Ching", evaluate_iching_retrieval),
    ]:
        print(f"\nEvaluating {name}...")
        r = func(col, k=k)
        results.append(r)
    ...
    total_h = sum(r["hits"] for r in results)
    print(f"\n{'='*60}")
    print(f"Overall precision@{k}: {total_h}/{total_q} = {total_h/total_q:.1%}")
    print(f"{'='*60}")
